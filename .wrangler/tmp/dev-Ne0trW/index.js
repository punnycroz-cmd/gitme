...
// .wrangler/tmp/bundle-0D1sV6/strip-cf-connecting-ip-header.js
...
async function listProjectFiles(FILES, user, proj) {
  ...
  return objects.map((obj) => obj.key.replace(prefix, "")).filter((path) => path && !isBlocked(path) && path !== ".dev.vars" && !path.endsWith("/.dev.vars")).sort();
}
...
async function handleDump(user, proj, request, env2, corsHeaders, forceJson = false) {
  ...
  const lastCommit = await env2.DB.prepare(
  ...
  if (mode === "delta") {
    if (hasCommits) {
      isDeltaMode = true;
      if (since) {
        const sinceCommit = await env2.DB.prepare("SELECT id, created_at FROM commits WHERE hash12 = ? AND user = ? AND proj = ?").bind(since, USER, proj).first();
        if (sinceCommit) {
          const commitsResult = await env2.DB.prepare("SELECT id FROM commits WHERE user = ? AND proj = ? AND created_at > ?").bind(USER, proj, sinceCommit.created_at).all();
          const commitIds = commitsResult.results.map((c) => c.id);
          if (commitIds.length > 0) {
            const placeholders = commitIds.map(() => "?").join(",");
            const filesResult = await env2.DB.prepare(`SELECT path, change_type FROM commit_files WHERE commit_id IN (${placeholders})`).bind(...commitIds).all();
            const pathMap = /* @__PURE__ */ new Map();
            for (const row of filesResult.results) {
              const prev = pathMap.get(row.path);
              if (row.change_type === "deleted") {
                if (prev === "added")
                  pathMap.delete(row.path);
                else
                  pathMap.set(row.path, "deleted");
              } else if (row.change_type === "added") {
                pathMap.set(row.path, "added");
              } else {
                if (prev !== "added")
                  pathMap.set(row.path, "modified");
              }
            }
          ...
          const filesResult = await env2.DB.prepare("SELECT path, change_type FROM commit_files WHERE commit_id = ?").bind(lastCommit.id).all();
  ...
  const allPaths = await listProjectFiles(env2.FILES, USER, proj);
  ...
  for (const f of changed) {
    if (pathFilter) {
      const cleanFilter = pathFilter.replace(/\/$/, "");
      const prefix = cleanFilter + "/";
      if (!f.path.startsWith(prefix) && f.path !== cleanFilter)
        continue;
    }
    if (f.change_type === "added")
      addedSet.add(f.path);
    else if (f.change_type === "modified")
      modifiedSet.add(f.path);
    else if (f.change_type === "deleted")
      deletedSet.add(f.path);
  }
  let filesToShow = [];
  let deletedFiles = [];
  let unchangedFiles = [];
  ...
  let totalBytes = 0;
  let fileCount = 0;
  const ALWAYS_INCLUDE = /* @__PURE__ */ new Set(["wrangler.toml"]);
  ...
  if (forceJson || wantsJson(request, searchParams)) {
    return jsonResponse({
      ...
      skipped,
      unchanged: includeUnchanged ? unchangedFiles : void 0,
      urls: {
...
`;
  ...
  if (unchangedFiles.length > 0) {
    textDump += `// Unchanged (${unchangedFiles.length} files not included): ${unchangedFiles.join(", ")}
`;
  }
...
Reply only with changed files using <code>// File: path</code></pre>
  ...
  const title2 = `TinyHub \u2014 ${USER}/${proj} (${isDeltaMode ? "delta" : "dump"})`;
  ...
  async fetch(request, env2) {
    ...
    try {
      ...
      if (method === "POST" && pathname.startsWith("/api/upload/")) {
        ...
        const lastCommitRow = await env2.DB.prepare(
        ...
        const batchStatements = [
          ...
          ).bind(commitId, USER, proj, commitHash, parentHash12, "Andie", commitMsg, JSON.stringify(stats), Math.floor(Date.now() / 1e3))
        ];
        for (const p of added) {
          batchStatements.push(
            env2.DB.prepare("INSERT INTO commit_files (commit_id, path, change_type) VALUES (?, ?, ?)").bind(commitId, p, "added")
          );
        }
        for (const p of modified) {
          batchStatements.push(
            env2.DB.prepare("INSERT INTO commit_files (commit_id, path, change_type) VALUES (?, ?, ?)").bind(commitId, p, "modified")
          );
        }
        for (const p of deleted) {
          batchStatements.push(
            env2.DB.prepare("INSERT INTO commit_files (commit_id, path, change_type) VALUES (?, ?, ?)").bind(commitId, p, "deleted")
          );
        }
        await env2.DB.batch(batchStatements);
      ...
      if (method === "POST" && pathname.startsWith("/api/commit/")) {
        ...
        const batchStatements = [
          env2.DB.prepare(
            "INSERT INTO commits (id, user, proj, hash12, parent_hash12, author, message, stats_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
          ).bind(commitId, USER, proj, commitHash, parentHash12, "Meta AI", commitMsg, JSON.stringify(stats), Math.floor(Date.now() / 1e3))
        ];
        for (const p of added) {
          batchStatements.push(
            env2.DB.prepare("INSERT INTO commit_files (commit_id, path, change_type) VALUES (?, ?, ?)").bind(commitId, p, "added")
      ...
      if (method === "GET" && pathname.startsWith("/api/bundle/")) {
        ...
        if (!proj)
          return new Response("Invalid project name", { status: 400, headers: corsHeaders });
        return await handleDump(USER, proj, request, env2, corsHeaders, true);
      }
      ...
      if (method === "GET" && pathname.startsWith("/dump/")) {
        ...
        return await handleDump(user, proj, request, env2, corsHeaders);
      }
...
// .wrangler/tmp/bundle-0D1sV6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0D1sV6/middleware-loader.entry.ts
